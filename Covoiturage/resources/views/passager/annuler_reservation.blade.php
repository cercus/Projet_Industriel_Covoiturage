...
@section('content')
    ...
    <form method="POST" action="{{route('acceptAnnulerReservation.store', $idReservation)}}" class="border border-dark" style="padding-top: 20px; padding-bottom: 20px;" >
        @csrf
        @if($errors->any())
            <div class="alert alert-warning">
                la reservation n'a pas été annulé &#9785; {{implode('', $errors->all('<div>:message</div>'))}}
            </div>
        @endif
